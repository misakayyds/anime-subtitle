...
def generate_ass_file(translated_data, output_path):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "Style: Default,微软雅黑,50,&H00FFFFFF,&H000000FF,&H00000000,&H00000000,1,0,0,0,100,100,0,0,1,3,0,2,20,20,30,1\n\n"
        )
        f.write(
...
async def translate_json(input_json_path, expected_output_ass=None):
    ...
    log_info(f"正在读取原轴数据: {input_json_path}")
    ...
    if not api_key or api_key.strip() == "":
        ...
        log_error(f"💡 请确保项目根目录下存在 `.env` 文件，并正确填写了 DEEPSEEK_API_KEY。")
        log_error(f"   项目根目录: {ENV_FILE.parent}")
        sys.exit(1)
...
返回示例: {{"1": {{"ja_corrected": "修正后的日文", "zh_translated": "中文翻译"}}}}"""
    ...
    log_info(f"🚀 开始超高速异步并发翻译（并发限制: {max_workers}）...")

    semaphore = asyncio.Semaphore(max_workers)

    async def process_chunk_async(idx, chunk, context_str):
        log_info(f"⏳ [队列中] 区块 {idx + 1}/{len(chunks)} 准备就绪...")
        async with semaphore:
            log_info(f"🚀 [处理中] 正在猛烈请求区块 {idx + 1}/{len(chunks)}...")
            chunk_dict = {item_id: item_data for item_id, item_data in chunk}
            input_text = "\n".join(
                [f"[ID: {item_id}] {item_data['ja_text']}" for item_id, item_data in chunk]
            )
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    log_info(f"✅ [成功] 区块 {idx + 1}/{len(chunks)} 翻译完成！")
                    ...
                    if attempt < max_retries - 1:
                        log_warning(
                            f"⚠️ [警告] 区块 {idx + 1}/{len(chunks)} 处理异常 ({str(e)})，正在进行第 {attempt + 1} 次重试..."
                        )
                        await asyncio.sleep(2**attempt)
                    else:
                        log_error(
                            f"❌ [崩溃降级] 区块 {idx + 1}/{len(chunks)} 连续 {max_retries} 次处理失败: {str(e)}，降级保留原文。"
                        )
                        for item_id, item_data in chunk_dict.items():
                            item_data["ja_corrected"] = item_data["ja_text"]
    ...
    log_info(f"生成 ASS 完美双语字幕文件中...")
    generate_ass_file(all_translated, output_ass_path)
    log_info(f"✅ 大功告成！双语字幕已保存为: {output_ass_path}")
